...
```{r, warning = FALSE, message = FALSE}
controles <-  unique(data_frame[data_frame$Company!="Vale.ADR",2])
controles_2 <- data.frame(unique(data_frame[,2]), 1:(length(controles)+1))
controles <- controles_2[-26,2]
names(controles_2) <- c("Company", "Index")

data_frame <- merge(data_frame, controles_2, by = "Company")

data_frame[1,1]
data_frame$data2 <- as.numeric(paste0(substr(data_frame[,2],1,4),substr(data_frame[,2],6,7),substr(data_frame[,2],9,10)))

data_frame$Company <- as.character(data_frame$Company)

data_frame2 <- c(data_frame[,1], data_frame[,5], data_frame[,6], data_frame[,3], data_frame[,4])

vale_controle <- dataprep(
  foo=data_frame,
  predictors = 'Volume', 
  predictors.op = 'mean',
  time.predictors.prior = 20140103:20151115,
  dependent = 'Return',
  unit.variable = 'Index',
  ...
  time.optimize.ssr = 20140103:20151115,
  time.plot = 20140103:20181231)
```
